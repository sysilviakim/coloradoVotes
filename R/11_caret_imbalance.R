...
multiClassSummary(temp2, lev = levels(t2$test$gen2020))
...
pdf_varimp(
  model_down,
  filename = here("fig", "gbm_caret_prAUC_downsample_10_varimp.pdf"),
  font = "CM Roman",
  labels = rev(
    c(
      "Reg. date",
...
pdf(
  here("fig", "gbm_caret_prAUC_downsample_10_roc_curve.pdf"),
  width = 6, height = 3
)
...
x <- x %>% mutate(.level = gsub("_", " ", .level))
pdf(
  here("fig", "gbm_caret_prAUC_downsample_10_pr_curve.pdf"),
  width = 6, height = 3
)
print(pdf_default(autoplot(x)))
dev.off()
